...
async function ensureStoreOwner(run, input) {
  ...
  await run(
    `INSERT INTO company_settings (
      id, name, app_name, description, tax_enabled, tax_percentage, currency_symbol, language, created_at, updated_at
    ) VALUES (1, ?, 'OpenPOS', 'Modern Point of Sale System', 1, 10.0, '$', 'en', ?, ?)
...
async function seedFreshStore(run, input) {
  ...
  await ensureStoreOwner(run, { ...input, now })
