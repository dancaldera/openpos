...
**Theme:** light with dark mode equivalent
...
The dark theme is not a new visual language; it is the same blank-page philosophy inverted into a low-glare black canvas. The system remains achromatic, flat, and type-led. White replaces black as the primary text and filled CTA color, borders become quiet charcoal separators, and color still appears only through editorial imagery. Avoid blue-tinted blacks, colored focus states, neon accents, and high-contrast card elevation.

### Dark Tokens — Colors

| Name | Value | Token | Role |
|------|-------|-------|------|
| Paper White | `#ffffff` | `--color-void` | Primary text, nav labels, filled CTA button background, icon fills — the inverted anchor for dark mode |
| Carbon Border | `#2a2a2a` | `--color-fog-border` | Dividing lines, card outlines, input borders, nav underlines — visible but intentionally quiet on near-black |
| Charcoal Hover | `#1f1f1f` | `--color-chalk` | Hover-state button backgrounds and subtle surface fills without introducing color |
| Silver | `#b3b3b3` | `--color-graphite` | Supporting body text, icon strokes, secondary labels |
| Smoke | `#7a7a7a` | `--color-ash` | Tertiary labels, disabled states, low-priority icon strokes |
| Obsidian | `#0f0f0f` | `--color-canvas` | Page background, card surfaces, all primary surfaces — near-black without a blue tint |

### Dark Component Rules

- **Page background:** `#0f0f0f` wall-to-wall; do not add tinted section bands.
- **Primary text:** `#ffffff`; keep typography weights at 400/500/600 only.
- **Secondary text:** `#b3b3b3`; tertiary and disabled copy use `#7a7a7a`.
- **Borders:** `#2a2a2a` only; avoid lighter gray borders that create boxed layouts.
- **Filled CTA:** `#ffffff` background with `#000000` text and the same 9999px radius.
- **Ghost buttons and inputs:** transparent background, `#2a2a2a` border, `#ffffff` text, `#1f1f1f` hover surface.
- **Editorial cards:** no dark card fill by default; image and type still float on the dark canvas.
- **Imagery:** retain soft editorial color, but reduce image brightness only if it overpowers adjacent white type.
- **Elevation:** replace light-mode shadow with either no shadow or `rgba(0, 0, 0, 0.35) 0px 4px 8px 0px`; never use glow.

### Dark Do's and Don'ts

#### Do

- Invert semantic tokens rather than creating separate component variants for every control
- Keep the same 9999px pill radius, 6.08px card/image radius, and 40px soft-button radius
- Use charcoal hover states sparingly so the interface still reads as open space
- Preserve the same spacing scale and section gaps as light mode
- Keep focus states monochrome: white outline or `#2a2a2a` outline with sufficient contrast

#### Don't

- Do not introduce blue-black, purple-black, or saturated dark backgrounds
- Do not use colored accent buttons in dark mode
- Do not add glow, blur, or gradient effects to compensate for the darker canvas
- Do not wrap editorial cards in elevated dark panels
- Do not lighten all borders equally; the border system should remain almost invisible

### Dark Quick Color Reference

- Text primary: #ffffff
- Text secondary: #b3b3b3
- Page background: #0f0f0f
- All borders/dividers: #2a2a2a
- Button hover surface: #1f1f1f
- CTA button fill: #ffffff (text: #000000)
...
[data-theme='dark'] {
  /* Colors */
  --color-void: #ffffff;
  --color-fog-border: #2a2a2a;
  --color-chalk: #1f1f1f;
  --color-graphite: #b3b3b3;
  --color-ash: #7a7a7a;
  --color-canvas: #0f0f0f;

  /* Shadows */
  --shadow-sm: rgba(0, 0, 0, 0.35) 0px 4px 8px 0px;
}
...
### Tailwind v4 Dark Mode Overrides
