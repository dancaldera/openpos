...
// The db-status test asserts the "no store configured" branch, so point the
// connections registry at an empty temp directory instead of the developer's
// real registry under apps/api/data/connections.
process.env.OPENPOS_CONNECTIONS_DIR = mkdtempSync(join(tmpdir(), 'openpos-api-status-'))
