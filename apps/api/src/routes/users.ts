...
usersRouter.get('/', async (c) => {
  ...
  const [countResult, users] = await Promise.all([
    ...
    query<DatabaseUser>('SELECT * FROM users WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT ? OFFSET ?', [
      limit,
      offset,
    ]),
  ])
...
usersRouter.post('/', async (c) => {
  ...
  const result = await execute(
    'INSERT INTO users (email, password, name, role, permissions, created_at, updated_at, password_hashed, pin_enabled, pin_hash) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)',
    [body.email.toLowerCase(), hashed, body.name, body.role, permissions, now, now, pinEnabled, pinHash],
  )
...
usersRouter.put('/:id', async (c) => {
  ...
  const body =
    await c.req.json<
      Partial<{ name: string; email: string; role: string; password: string; pinEnabled: boolean; pin: string }>
    >()
  const now = new Date().toISOString()
  ...
  if (body.pinEnabled === false) {
    fields.push('pin_enabled = ?', 'pin_hash = ?')
    values.push(0, null)
  } else if (body.pinEnabled === true || body.pin) {
    if (body.pin) {
      const pinError = validatePin(body.pin)
      if (pinError) return c.json({ error: pinError }, 400)
      const hashed = await bcrypt.hash(body.pin, BCRYPT_ROUNDS)
      fields.push('pin_enabled = ?', 'pin_hash = ?')
      values.push(1, hashed)
    } else {
      const existing = await query<DatabaseUser>('SELECT pin_hash FROM users WHERE id = ? LIMIT 1', [id])
      if (!existing[0]?.pin_hash) {
        return c.json({ error: 'PIN must be exactly 6 digits' }, 400)
      }
      fields.push('pin_enabled = ?')
      values.push(1)
    }
  }
...
function toUser(u: DatabaseUser) {
  return {
    ...
    pinEnabled: Boolean(u.pin_enabled) && Boolean(u.pin_hash),
  }
}
