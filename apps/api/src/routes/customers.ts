...
function normalizeTags(value: unknown): string[] | undefined {
  ...
  const normalizeEntries = (entries: unknown[]) => {
    const uniqueTags = new Set(entries.map((entry) => normalizeText(entry)).filter((entry) => entry.length > 0))
...
async function findExistingCustomer(id: number) {
  const rows = await query<DatabaseCustomer>('SELECT * FROM customers WHERE id = ? AND deleted_at IS NULL LIMIT 1', [
    id,
  ])
  return rows[0] ?? null
}
