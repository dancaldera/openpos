...
analyticsRouter.get('/summary', async (c) => {
  const [orderStats, avgValue, profitStats, customerCount, newCustomers] = await Promise.all([
    query<{
      total_orders: number
    ...
    query<{ avg_order_value: number }>("SELECT AVG(total) as avg_order_value FROM orders WHERE status = 'completed'"),
