/**
 * Auth routes
 *
 * POST /api/auth/login                  — verify credentials, return JWT
 * POST /api/auth/hash                   — bcrypt hash a password (replaces Rust invoke on web)
 * POST /api/auth/verify                 — bcrypt verify a password (replaces Rust invoke on web)
 * GET  /api/auth/me                     — return the current user from JWT (protected)
 * POST /api/auth/verify-internal-secret — verify the operator INTERNAL_SECRET (public)
 * POST /api/auth/admin-reset-password   — reset password with INTERNAL_SECRET (public)
 */

import { parseConnectionKey } from '@openpos/data'
import bcrypt from 'bcryptjs'
import { Hono } from 'hono'
import {
  connectionErrorStatus,
  readAssignedConnection,
  readCurrentConnectionMeta,
  registerConnection,
  resolveDataPlane,
} from '../lib/connection.js'
import { validatePasswordStrength, validatePin } from '../lib/password.js'
import { isValidEmail, verifyInternalSecret } from '../lib/secrets.js'
import { createDataPlaneClient, execute, query, runWithDataPlane } from '../lib/turso.js'
import { authMiddleware, type JwtPayload, signToken } from '../middleware/auth.js'

const BCRYPT_ROUNDS = 12
const INVALID_ADMIN_SECRET_ERROR = 'Invalid internal secret'

interface DatabaseUser {
  id: number
  email: string
  password: string
  name: string
  role: 'admin' | 'manager' | 'user'
  permissions: string
  created_at: string
  last_login?: string
  deleted_at?: string
  password_hashed?: number
  pin_enabled?: number
  pin_hash?: string | null
}

const INVALID_PIN_ERROR = 'Invalid PIN'

function userHasPin(user: DatabaseUser): boolean {
  return Boolean(user.pin_enabled) && Boolean(user.pin_hash)
}

function toLoginUser(dbUser: DatabaseUser) {
  const permissions: string[] = JSON.parse(dbUser.permissions)
  return {
    id: dbUser.id.toString(),
    email: dbUser.email,
    name: dbUser.name,
    role: dbUser.role,
    permissions,
    createdAt: dbUser.created_at,
    lastLogin: dbUser.last_login,
    pinEnabled: userHasPin(dbUser),
  }
}

async function issueLoginResponse(dbUser: DatabaseUser) {
  await query('UPDATE users SET last_login = ? WHERE id = ?', [new Date().toISOString(), dbUser.id])

  const user = toLoginUser(dbUser)
  const connection = await readCurrentConnectionMeta()
  const token = signToken({
    sub: dbUser.id.toString(),
    email: dbUser.email,
    name: dbUser.name,
    role: dbUser.role,
    permissions: user.permissions,
    ...(connection ? { connectionKey: connection.key } : {}),
  })

  return { token, user }
}

// ---------------------------------------------------------------------------
export const authRouter = new Hono()

// ---------------------------------------------------------------------------
// GET /api/auth/users  (public - for login screen user list)
// ---------------------------------------------------------------------------
authRouter.get('/users', async (c) => {
  // Public endpoint - no auth required
  // Returns only safe fields: id, name, email, role, pinEnabled (no passwords or PIN hashes)
  const users = await query<Pick<DatabaseUser, 'id' | 'email' | 'name' | 'role' | 'pin_enabled'>>(
    'SELECT id, email, name, role, pin_enabled FROM users WHERE deleted_at IS NULL ORDER BY name ASC',
  )

  const safeUsers = users.map((user) => ({
    id: user.id.toString(),
    email: user.email,
    name: user.name,
    role: user.role,
    pinEnabled: Boolean(user.pin_enabled),
  }))

  return c.json({ users: safeUsers })
})

// ---------------------------------------------------------------------------
// POST /api/auth/login
// ---------------------------------------------------------------------------
authRouter.post('/login', async (c) => {
  let body: { email?: string; password?: string; userId?: string | number; pin?: string }
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON body' }, 400)
  }

  const pin = typeof body.pin === 'string' ? body.pin : ''
  const userIdRaw = body.userId
  const isPinLogin = userIdRaw !== undefined && userIdRaw !== null && userIdRaw !== '' && pin.length > 0
  const email = (body.email || '').trim()
  const password = body.password || ''

  if (isPinLogin) {
    if (validatePin(pin)) {
      return c.json({ error: INVALID_PIN_ERROR }, 401)
    }
  } else if (!email || !password) {
    return c.json({ error: 'email and password are required' }, 400)
  }

  const connectionKey =
    parseConnectionKey(c.req.header('X-OpenPOS-Connection') || '') || (await readAssignedConnection())?.key || ''
  if (!connectionKey) {
    return c.json({ error: 'Store connection required' }, 401)
  }

  let dataPlane = await resolveDataPlane(connectionKey)
  if (!dataPlane) {
    if (isPinLogin) {
      return c.json({ error: 'Store connection required' }, 401)
    }

    try {
      const registered = await registerConnection({
        key: connectionKey,
        storeName: 'OpenPOS',
        adminName: email.split('@')[0] || 'Admin',
        adminEmail: email,
        adminPassword: password,
      })
      dataPlane = registered.dataPlane
    } catch (error) {
      return c.json(
        { error: error instanceof Error ? error.message : 'Unable to register store' },
        connectionErrorStatus(error),
      )
    }
  }

  return runWithDataPlane(createDataPlaneClient(dataPlane), async () => {
    if (isPinLogin) {
      const userId = Number.parseInt(String(userIdRaw), 10)
      if (!Number.isInteger(userId) || userId < 1) {
        return c.json({ error: INVALID_PIN_ERROR }, 401)
      }

      const users = await query<DatabaseUser>('SELECT * FROM users WHERE id = ? AND deleted_at IS NULL LIMIT 1', [
        userId,
      ])
      const dbUser = users[0]
      if (!dbUser || !userHasPin(dbUser) || !dbUser.pin_hash) {
        return c.json({ error: INVALID_PIN_ERROR }, 401)
      }

      const pinValid = await bcrypt.compare(pin, dbUser.pin_hash)
      if (!pinValid) {
        return c.json({ error: INVALID_PIN_ERROR }, 401)
      }

      return c.json(await issueLoginResponse(dbUser))
    }

    const users = await query<DatabaseUser>('SELECT * FROM users WHERE email = ? AND deleted_at IS NULL LIMIT 1', [
      email.toLowerCase(),
    ])

    if (users.length === 0) {
      return c.json({ error: 'Invalid email or password' }, 401)
    }

    const dbUser = users[0]
    const isHashed = dbUser.password_hashed === 1

    let passwordValid: boolean
    if (isHashed) {
      passwordValid = await bcrypt.compare(password, dbUser.password)
    } else {
      // Plain-text fallback (legacy users)
      passwordValid = dbUser.password === password
      if (passwordValid) {
        // Lazy-migrate to bcrypt
        const hashed = await bcrypt.hash(password, BCRYPT_ROUNDS)
        await query('UPDATE users SET password = ?, password_hashed = 1 WHERE id = ?', [hashed, dbUser.id])
      }
    }

    if (!passwordValid) {
      return c.json({ error: 'Invalid email or password' }, 401)
    }

    return c.json(await issueLoginResponse(dbUser))
  })
})

// ---------------------------------------------------------------------------
// POST /api/auth/hash  (internal — replaces Rust invoke in web context)
// ---------------------------------------------------------------------------
authRouter.post('/hash', async (c) => {
  let body: { password?: string }
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON body' }, 400)
  }

  if (!body.password) {
    return c.json({ error: 'password is required' }, 400)
  }

  const hash = await bcrypt.hash(body.password, BCRYPT_ROUNDS)
  return c.json({ hash })
})

// ---------------------------------------------------------------------------
// POST /api/auth/verify  (internal — replaces Rust invoke in web context)
// ---------------------------------------------------------------------------
authRouter.post('/verify', async (c) => {
  let body: { password?: string; hash?: string }
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON body' }, 400)
  }

  if (!body.password || !body.hash) {
    return c.json({ error: 'password and hash are required' }, 400)
  }

  const valid = await bcrypt.compare(body.password, body.hash)
  return c.json({ valid })
})

// ---------------------------------------------------------------------------
// GET /api/auth/me  (protected)
// ---------------------------------------------------------------------------
authRouter.get('/me', authMiddleware, (c) => {
  // biome-ignore lint/suspicious/noExplicitAny: jwtPayload set by authMiddleware
  const payload = (c as any).get('jwtPayload') as JwtPayload
  return c.json({ user: payload })
})

// ---------------------------------------------------------------------------
// POST /api/auth/admin-reset-password  (public — operator INTERNAL_SECRET)
// ---------------------------------------------------------------------------
authRouter.post('/admin-reset-password', async (c) => {
  let body: { email?: string; newPassword?: string; internalSecret?: string }
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON body' }, 400)
  }

  const email = body.email?.trim().toLowerCase() || ''
  const newPassword = body.newPassword || ''
  const internalSecret = body.internalSecret || ''

  if (!email || !isValidEmail(email) || !newPassword || !internalSecret) {
    return c.json({ error: 'email, newPassword and internalSecret are required' }, 400)
  }

  const strengthError = validatePasswordStrength(newPassword)
  if (strengthError) {
    return c.json({ error: strengthError }, 400)
  }

  let secretValid = false
  try {
    secretValid = verifyInternalSecret(internalSecret)
  } catch (error) {
    console.error('[Auth] INTERNAL_SECRET is not configured:', error)
    return c.json({ error: 'Internal secret is not configured' }, 503)
  }

  if (!secretValid) {
    return c.json({ error: INVALID_ADMIN_SECRET_ERROR }, 401)
  }

  const users = await query<{ id: number }>('SELECT id FROM users WHERE email = ? AND deleted_at IS NULL LIMIT 1', [
    email,
  ])
  if (users.length === 0) {
    return c.json({ error: 'User not found' }, 404)
  }

  const hashed = await bcrypt.hash(newPassword, BCRYPT_ROUNDS)
  await execute('UPDATE users SET password = ?, password_hashed = 1, updated_at = ? WHERE id = ?', [
    hashed,
    new Date().toISOString(),
    users[0].id,
  ])

  return c.json({ success: true })
})

// ---------------------------------------------------------------------------
// POST /api/auth/verify-internal-secret  (public — gate for operator actions)
// ---------------------------------------------------------------------------
authRouter.post('/verify-internal-secret', async (c) => {
  let body: { internalSecret?: string }
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON body' }, 400)
  }

  if (!body.internalSecret) {
    return c.json({ error: 'internalSecret is required' }, 400)
  }

  let secretValid = false
  try {
    secretValid = verifyInternalSecret(body.internalSecret)
  } catch (error) {
    console.error('[Auth] INTERNAL_SECRET is not configured:', error)
    return c.json({ error: 'Internal secret is not configured' }, 503)
  }

  if (!secretValid) {
    return c.json({ error: INVALID_ADMIN_SECRET_ERROR }, 401)
  }

  return c.json({ valid: true })
})
