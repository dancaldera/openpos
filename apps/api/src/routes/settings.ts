...
settingsRouter.get('/public', async (c) => {
  ...
  // theme_mode, theme_palette (cosmetic, drives the pre-login look and feel)
  const rows = await query<DatabaseCompanySettings>(
    'SELECT name, app_name, language, currency_symbol, logo_url, theme_mode, theme_palette FROM company_settings LIMIT 1',
  )
...
settingsRouter.put('/database', async (c) => {
  ...
  try {
    const resolvedAuthToken =
      authToken || (existing?.auth_token_encrypted ? decryptSecret(existing.auth_token_encrypted) : '')
    const resolvedApiToken =
      apiToken || (existing?.api_token_encrypted ? decryptSecret(existing.api_token_encrypted) : '')
