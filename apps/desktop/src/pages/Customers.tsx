...
function serializeCustomerForm(formData: CustomerFormData): string {
  return JSON.stringify(formData)
}

function CustomerFormPage({ customer, onBack, onSaved }: CustomerFormPageProps) {
  ...
  const [initialFormSnapshot, setInitialFormSnapshot] = useState(() => serializeCustomerForm(createEmptyFormData()))
  ...
  useEffect(() => {
    if (customer) {
      ...
      setInitialFormSnapshot(serializeCustomerForm(nextFormData))
    } else {
  ...
  const isDirty = useMemo(
    () => serializeCustomerForm(formData) !== initialFormSnapshot,
  ...
  return (
    <div class="mx-auto max-w-3xl">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex min-w-0 items-center gap-2">
            ...
            <div class="min-w-0">
              ...
              <p class="text-sm text-graphite">{t('customers.formPageHint')}</p>
            </div>
          </div>
                ...
                <div class="md:col-span-2">
                  <label class="flex items-center space-x-3 cursor-pointer">
                    <input
                      ...
                      class="h-5 w-5 rounded border-fog-border bg-canvas text-void focus:ring-accent "
                    />
                    <div>
      ...
      <DialogConfirm
        ...
        title={t('customers.unsavedChangesTitle')}
        message={t('customers.unsavedChangesMessage')}
        confirmText={t('customers.discardChanges')}
...
export default function Customers() {
  ...
  const handleSaveCustomer = async (_customer: Customer) => {
    await loadCustomers()
  ...
  if (view === 'form') {
    return <CustomerFormPage customer={formCustomer} onBack={handleCloseCustomerForm} onSaved={handleSaveCustomer} />
  }
