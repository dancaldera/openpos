...
import { setSidebarCollapsed, sidebarCollapsed } from '../stores/ui/sidebarStore'
  ...
  return (
    <div class="flex h-screen bg-canvas text-void">
      <Sidebar
        ...
        toggleLabel={t('navigation.toggleSidebar')}
        ...
        footer={isDesktop ? ({ collapsed }) => <UpdateBadge placement="sidebar" collapsed={collapsed} /> : undefined}
      />

      {/* The desktop sidebar is hidden below md; keep updates reachable until the mobile drawer opens. */}
      {!isMobileSidebarOpen && (
        <div class="md:hidden">
          <UpdateBadge />
        </div>
      ...
      <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-canvas">
          <div class="container mx-auto px-4 pb-16 pt-4 md:px-6 md:pb-20 md:pt-6">{children}</div>
        </main>
      </div>
