...
import { sidebarCollapsed } from '../../stores/ui/sidebarStore'
...
const FALLBACK_RELEASE_URL = 'https://releases.openpos.xyz'
...
export function UpdateBadge({ placement = 'floating', collapsed: collapsedOverride }: UpdateBadgeProps = {}) {
  if (!isDesktop) return null

  const isMac = typeof window !== 'undefined' && window.__OPENPOS_DESKTOP__?.platform === 'darwin'
  ...
  const isCollapsed = isSidebar && (collapsedOverride ?? sidebarCollapsed.value)
  const updateIconClass = isSidebar ? 'h-5 w-5' : 'h-4 w-4'
  ...
  return (
    <div
      ref={wrapperRef}
      class={`no-drag flex flex-col gap-2 ${
        isSidebar ? 'relative w-full items-start' : 'fixed bottom-4 left-4 z-50 items-start'
      }`}
    >
      {popoverOpen.value && (
      ...
      {popoverOpen.value && (
        <div
          class={`absolute bottom-full left-0 z-50 mb-2 w-72 overflow-hidden rounded-cards border ${viewModel.borderClass} bg-canvas text-xs text-graphite shadow-sm`}
        >
          ...
          <div class="px-4 pb-3 flex flex-col gap-2">
            ...
            {viewModel.canAutoInstall && (
              <button
                ...
                class="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-buttons bg-accent text-canvas text-xs font-medium transition-colors disabled:opacity-50"
              >
      ...
      <button
        ...
        class={`relative flex min-h-9 items-center rounded-buttons border ${viewModel.borderClass} bg-canvas/90 shadow-sm backdrop-blur-sm cursor-pointer select-none transition-[border-color,box-shadow,opacity] duration-200 hover:border-graphite hover:shadow-md ${
          isSidebar ? `h-9 gap-2 ${isMac ? 'pl-[13px]' : 'pl-[9px]'} pr-3 py-1.5` : 'gap-2 px-3 py-1.5'
        }`}
        title={labels.appUpdate}
        aria-label={isCollapsed ? labels.appUpdate : undefined}
        aria-expanded={popoverOpen.value}
      >
        {checking || downloading || installing ? (
          <SpinnerIcon class={`${updateIconClass} animate-spin ${viewModel.iconColorClass}`} />
        ) : (
          <span class={`relative inline-flex ${updateIconClass} shrink-0`}>
            <RocketIcon class={`${updateIconClass} ${viewModel.iconColorClass}`} />
