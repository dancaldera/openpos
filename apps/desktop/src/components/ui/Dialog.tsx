...
export function Dialog({ isOpen, onClose, title, children, size = 'md', closeOnOutsideClick = true }: DialogProps) {
  ...
  return (
    <div class="no-drag fixed inset-0 z-[100] overflow-y-auto">
      <button
        type="button"
        class={clsx(
          ...
          {title && (
            <div class="flex items-center justify-between p-6 border-b border-fog-border">
              ...
              <button
                type="button"
                onClick={onClose}
                class="p-2 text-graphite hover:text-void focus:outline-none focus:ring-2 focus:ring-accent rounded-buttons"
              >
