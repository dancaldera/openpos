...
import { normalizeThemeMode, normalizeThemePalette, type ThemePalette } from '../stores/theme/palettes'
import type { ThemePreference } from '../stores/theme/themeStore'
...
export class CompanySettingsService {
  private convertDbSettings(dbSettings: DatabaseCompanySettings): CompanySettings {
    return {
      ...
      themeMode: normalizeThemeMode(dbSettings.theme_mode),
      themePalette: normalizeThemePalette(dbSettings.theme_palette),
  ...
  async updateSettings(
    ...
    try {
      ...
      if (updates.themeMode !== undefined) {
        ...
        updateValues.push(updates.themeMode)
      }

      if (updates.themePalette !== undefined) {
        updateFields.push('theme_palette = ?')
        updateValues.push(updates.themePalette)
      }
      ...
      if (updateFields.length > 1) {
        ...
        if (isDesktop) {
          // Push the queued write immediately instead of waiting for the
          // background sync poll. Best effort: offline it replicates later.
    ...
    try {
      ...
      await execute(
        `UPDATE company_settings SET
         ...
         phone = ?, email = ?, website = ?, theme_mode = ?, theme_palette = ?, updated_at = ?
         WHERE id = ?`,
        [
          'Titanic POS',
          'OpenPOS',
