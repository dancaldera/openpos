...
export class ProductService {
  ...
  async deleteProduct(id: string): Promise<{ success: boolean; error?: string }> {
    try {
      const result = await execute('DELETE FROM products WHERE id = ?', [parseInt(id, 10)])
  ...
  async convertToConfigurable(
    ...
    try {
      ...
      await execute('UPDATE products SET variant_type = ?, updated_at = ? WHERE id = ?', [
        'configurable',
  ...
  async convertToSimple(productId: string): Promise<{ success: boolean; product?: Product; error?: string }> {
    try {
      ...
      await execute('DELETE FROM product_variant_settings WHERE product_id = ?', [parseInt(productId, 10)])

      // Update product to simple
      await execute('UPDATE products SET variant_type = ?, default_variant_id = NULL, updated_at = ? WHERE id = ?', [
        'simple',
        new Date().toISOString(),
        parseInt(productId, 10),
      ])
  ...
  async setDefaultVariant(productId: string, variantId: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      await execute('UPDATE products SET default_variant_id = ?, updated_at = ? WHERE id = ?', [
        parseInt(variantId, 10),
        new Date().toISOString(),
        parseInt(productId, 10),
      ])
