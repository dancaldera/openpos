...
export class ProductVariantsService {
  ...
  async deleteAttribute(id: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const result = await execute('DELETE FROM product_attributes WHERE id = ?', [parseInt(id, 10)])
  ...
  async deleteVariant(id: string): Promise<{ success: boolean; error?: string }> {
    try {
      const result = await execute('DELETE FROM product_variants WHERE id = ?', [parseInt(id, 10)])
  ...
  async updateVariantStock(id: string, quantity: number): Promise<{ success: boolean; error?: string }> {
    try {
      const result = await execute('UPDATE product_variants SET stock = stock + ?, updated_at = ? WHERE id = ?', [
        quantity,
  ...
  async deleteVariantSettings(productId: string): Promise<{ success: boolean; error?: string }> {
    try {
      const result = await execute('DELETE FROM product_variant_settings WHERE product_id = ?', [
        parseInt(productId, 10),
      ])
