...
Prerequisites: [Node.js](https://nodejs.org/) 22.13+ and [pnpm](https://pnpm.io/).
...
OpenPOS is a pnpm workspace monorepo: `apps/desktop` (Electron + Preact POS client), `apps/api` (Hono backend), `apps/landing` (Astro marketing site), and shared code in `packages/`.

Run commands from the repository root:

```bash
pnpm run dev          # desktop app (renderer on http://localhost:1420)
pnpm run dev:api      # API server
pnpm run dev:landing  # landing site
```

## Before opening a PR

```bash
pnpm run check  # TypeScript + Biome validation
...
- **Tests**: use Vitest, colocated with the code as `*.test.ts`. Add focused tests for the route handlers, services, sync flows, or utilities you touch.
